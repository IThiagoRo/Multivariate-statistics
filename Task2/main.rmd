...
$$S^{-\frac{1}{2}}[(\bar{X_n}-\bar{Y_m}) - (\mu_H -\mu_M)] \stackrel{d}{\rightarrow} N_p(\mathbf{0}, \boldsymbol{I_p})$$
...
$$H_0: \mu_h = \mu_m \ vs \ H_a: \mu_h \neq \mu_m $$
Que es equivalente a probar:
$$H_0: \mu_h - \mu_m = \delta_0 \ vs \ H_a: \mu_h - \mu_m \neq \delta_0 $$
...
Se obtine un $X_C = 1050.692 > \chi_{0.05}^{2}(8) = 16.91898$ con una significancia de $\alpha = 0.05$ nos permite rechazar $H_0$ y concluir que el vector $X$ no son suficiente para poder discriminar entre Hombres y Mujeres.
